...
```{r}
quiz_tries <- question_submit_tries(quest_data)
```
...
res <- plot_exercise_prop_correct(ex_data)
res$plot
```
...
for (track_sess_id in sort(unique(tracking_data$track_sess_id))) {
    print(plot_mouse_tracks_for_tracking_session(tracking_data, track_sess_id))
}
```
